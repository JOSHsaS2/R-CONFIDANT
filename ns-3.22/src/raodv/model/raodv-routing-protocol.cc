 ...
 *      http://core.it.uu.se/core/index.php/RAODV-UU
 *
...
//-----------------------------------------------------------------------------
/// Tag used by RAODV implementation
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::raodv::DeferredRouteOutputTag").SetParent<Tag> ()
      .SetParent<Tag> ()
...
RoutingProtocol::RoutingProtocol () :
  ...
  HelloInterval (Seconds (5)),  //modified::originally 1
  AllowedHelloLoss (10),        //modified::originall 2
  ...
  EnableHello (false),
  m_routingTable (DeletePeriod),								//XJLM::irrelevant
...
{
  static TypeId tid = TypeId ("ns3::raodv::RoutingProtocol")
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream) const
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId () << " Time: " << Simulator::Now ().GetSeconds () << "s ";
  //m_rankBuffer.Print (stream);	XJLM::irrelevant
}
...
RoutingProtocol::Start ()
{
  ...
  m_successfullPacketsRecieved=0;
}
                              ...
                              Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
  ...
  if (m_rankBuffer.LookupValidRoute (dst, rt))				//XJLM::TODO
    {
      route = rt.GetRoute ();
      NS_ASSERT (route != 0);
  ...
  if (result)
    {
      ...
      RoutingTableEntry rt;
      bool result = m_rankBuffer.LookupRoute (header.GetDestination (), rt);
...
{
  m_totalPacketsReceived++; //DEBUG
  //cout << m_ipv4->GetObject<Node>()->GetId() << " :: TOTAL PACKETS RECEIVED = [" << m_totalPacketsReceived << "]" << endl;
            ...
            if (header.GetTtl () > 1)
              {
                ...
                if (m_rankBuffer.LookupRoute (dst, toBroadcast))			//XJLM::TODO
                  {
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      m_successfullPacketsRecieved++;

      UpdateRouteLifeTime (origin, ActiveRouteTimeout);
      RoutingTableEntry toOrigin;
      if (m_rankBuffer.LookupValidRoute (origin, toOrigin))				// XJLM::TODO
        {
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  Ipv4Address dst = header.GetDestination ();
  Ipv4Address origin = header.GetSource ();
  m_rankBuffer.Purge ();															//XJLM::TODO
  RoutingTableEntry toDst;

        //BLACKHOLE SIMULATION
        if( IsMalicious )   // CHANGE ID HERE
          {//When malicious node receives packet it drops the packet.
            std :: cout << m_ipv4->GetObject<Node>()->GetId() <<" :: Launching Blackhole Attack! Packet dropped . . . \n";            

            NS_LOG_LOGIC ("BLACKHOLE::Send RERR message.");
            NS_LOG_DEBUG ("Drop packet " << p->GetUid () << " because BLACKHOLE!.");
            SendRerrWhenNoRouteToForward (dst, 0, origin);
            m_rankBuffer.Clear();
            return false; 
          }
        //endBlackHoleSimulation

  if (m_rankBuffer.LookupRoute (dst, toDst))					//XJLM::TODO
    {
      if (toDst.GetFlag () == VALID)
        {
           ...
           */
          RoutingTableEntry toOrigin;
          m_rankBuffer.LookupRoute (origin, toOrigin);
          UpdateRouteLifeTime (toOrigin.GetNextHop (), ActiveRouteTimeout);
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  m_rankBuffer.AddRoute (rt);						//XJLM::TODO
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), RAODV_PORT));
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      m_htimer.Cancel ();
      m_nb.Clear ();
      m_rankBuffer.Clear ();							//XJLM::TODO
      return;
    }
  m_rankBuffer.DeleteAllRoutesFromInterface (m_ipv4->GetAddress (i, 0));		//XJLM::TODO
}
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("RAODV does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    {
      m_rankBuffer.DeleteAllRoutesFromInterface (address);			//XJLM::TODO
      socket->Close ();
      m_socketAddresses.erase (socket);
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  RoutingTableEntry rt;
  if (m_rankBuffer.LookupRoute (dst, rt))								//XJLM::TODO
    {
      ...
      m_rankBuffer.Update (rt);								//XJLM::TODO
    }
  else
    {
      rreqHeader.SetUnknownSeqno (true);
      ...
      m_rankBuffer.AddRoute (newEntry);								//XJLM::TODO
    }
...
RoutingProtocol::ScheduleRreqRetry (Ipv4Address dst)
{
  ...
  RoutingTableEntry rt;
  m_rankBuffer.LookupRoute (dst, rt);								//XJLM::TODO								//XJLM::TODO
  rt.IncrementRreqCnt ();
  m_rankBuffer.Update (rt);								//XJLM::TODO
...
RoutingProtocol::RecvRaodv (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("RAODV node " << this << " received a RAODV packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("RAODV message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
...
RoutingProtocol::UpdateRouteLifeTime (Ipv4Address addr, Time lifetime)
{
  ...
  RoutingTableEntry rt;
  if (m_rankBuffer.LookupRoute (addr, rt))								//XJLM::TODO
    {
      if (rt.GetFlag () == VALID)
        {
...
RoutingProtocol::UpdateRouteToNeighbor (Ipv4Address sender, Ipv4Address receiver)
{
  ...
  if (!m_rankBuffer.LookupRoute (sender, toNeighbor))								//XJLM::TODO
    {
  ...
  else
    {
      ...
      if (toNeighbor.GetValidSeqNo () && (toNeighbor.GetHop () == 1) && (toNeighbor.GetOutputDevice () == dev))
      ...
      else
        {
          ...
          m_rankBuffer.Update (newEntry);								//XJLM::TODO
        }
    }
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  if (m_rankBuffer.LookupRoute (src, toPrev))								//XJLM::TODO
    {
   ...
   */
  RoutingTableEntry toOrigin;
  if (!m_rankBuffer.LookupRoute (origin, toOrigin))								//XJLM::TODO
    {
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
  ...
  else
    {
      ...
      m_rankBuffer.Update (toOrigin);								//XJLM::TODO
      //m_nb.Update (src, Time (AllowedHelloLoss * HelloInterval));
    }


  RoutingTableEntry toNeighbor;
  if (!m_rankBuffer.LookupRoute (src, toNeighbor))								//XJLM::TODO
    {
  ...
  else
    {
      ...
      m_rankBuffer.Update (toNeighbor);								//XJLM::TODO
    }
  m_nb.Update (src, Time (AllowedHelloLoss * HelloInterval));
  ...
  if (IsMyOwnAddress (rreqHeader.GetDst ()))
    {
      m_rankBuffer.LookupRoute (origin, toOrigin);								//XJLM::TODO
  ...
  // if (IsMalicious || m_rankBuffer.LookupRoute (dst, toDst))								//Shalini Satre
  if (m_rankBuffer.LookupRoute (dst, toDst))
    {
      /*
       ...
       */
      // if (IsMalicious || ((rreqHeader.GetUnknownSeqno () || (int32_t (toDst.GetSeqNo ()) - int32_t (rreqHeader.GetDstSeqno ()) >= 0))               //Shalini Satre
      if ((rreqHeader.GetUnknownSeqno () || (int32_t (toDst.GetSeqNo ()) - int32_t (rreqHeader.GetDstSeqno ()) >= 0))
          && toDst.GetValidSeqNo () )
        {
          // if (IsMalicious || (!rreqHeader.GetDestinationOnly () && toDst.GetFlag () == VALID) )               //Shalini Satre
...
RoutingProtocol::SendReplyByIntermediateNode (RoutingTableEntry & toDst, RoutingTableEntry & toOrigin, bool gratRep)
{
  ...
  if (toDst.GetHop () == 1)
    {
      ...
      m_rankBuffer.LookupRoute (toOrigin.GetNextHop (), toNextHop);								//XJLM::TODO
  ...
  m_rankBuffer.Update (toDst);								//XJLM::TODO
  m_rankBuffer.Update (toOrigin);								//XJLM::TODO
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RoutingTableEntry toNeighbor;
  m_rankBuffer.LookupRoute (neighbor, toNeighbor);								//XJLM::TODO
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  if (IsMyOwnAddress (rrepHeader.GetOrigin ()))
    {
      ...
      m_rankBuffer.LookupRoute (dst, toDst);								//XJLM::TODO
      SendPacketFromQueue (dst, toDst.GetRoute ());
      return;
    }

  RoutingTableEntry toOrigin;
  if (!m_rankBuffer.LookupRoute (rrepHeader.GetOrigin (), toOrigin) || toOrigin.GetFlag () == IN_SEARCH)								//XJLM::TODO
    {
  ...
  if (m_rankBuffer.LookupValidRoute (rrepHeader.GetDst (), toDst))								//XJLM::TODO
    {
      toDst.InsertPrecursor (toOrigin.GetNextHop ());
      m_rankBuffer.Update (toDst);								//XJLM::TODO

      RoutingTableEntry toNextHopToDst;
      m_rankBuffer.LookupRoute (toDst.GetNextHop (), toNextHopToDst);								//XJLM::TODO
      toNextHopToDst.InsertPrecursor (toOrigin.GetNextHop ());
      m_rankBuffer.Update (toNextHopToDst);								//XJLM::TODO

      toOrigin.InsertPrecursor (toDst.GetNextHop ());
      m_rankBuffer.Update (toOrigin);								//XJLM::TODO

      RoutingTableEntry toNextHopToOrigin;
      m_rankBuffer.LookupRoute (toOrigin.GetNextHop (), toNextHopToOrigin);								//XJLM::TODO
      toNextHopToOrigin.InsertPrecursor (toDst.GetNextHop ());
      m_rankBuffer.Update (toNextHopToOrigin);								//XJLM::TODO
    }
...
RoutingProtocol::RecvReplyAck (Ipv4Address neighbor)
{
  NS_LOG_FUNCTION (this);
  RoutingTableEntry rt;
  if(m_rankBuffer.LookupRoute (neighbor, rt))								//XJLM::TODO
    {
...
RoutingProtocol::ProcessHello (RrepHeader const & rrepHeader, Ipv4Address receiver )
{
   ...
   */
  RoutingTableEntry toNeighbor;
  if (!m_rankBuffer.LookupRoute (rrepHeader.GetDst (), toNeighbor))								//XJLM::TODO
    {
      Ptr<NetDevice> dev = m_ipv4->GetNetDevice (m_ipv4->GetInterfaceForAddress (receiver));
  ...
  else
    {
      ...
      m_rankBuffer.Update (toNeighbor);								//XJLM::REDIRECT
    }
  if (EnableHello)
    {
...
RoutingProtocol::RecvError (Ptr<Packet> p, Ipv4Address src )
{
  ...
  m_rankBuffer.GetListOfDestinationWithNextHop (src, dstWithNextHopSrc);	//XJLM::REDIRECT

//XJLM::RAODV FUNCTION
  m_rankBuffer.deRank(src); 
  cout << "\t" << m_ipv4->GetObject<Node>()->GetId() << " :: deRank(" << src << ")" << endl;
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end ();)
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        ...
        {
          RoutingTableEntry toDst;
          m_rankBuffer.LookupRoute (i->first, toDst);								//XJLM::TODO
  ...
  m_rankBuffer.InvalidateRoutesWithDst (src, unreachable);								//XJLM::TODO 20150603WED
}
...
RoutingProtocol::RouteRequestTimerExpire (Ipv4Address dst)
{
  NS_LOG_LOGIC (this);
  RoutingTableEntry toDst;
  if (m_rankBuffer.LookupValidRoute (dst, toDst))								//XJLM::TODO
    {
      SendPacketFromQueue (dst, toDst.GetRoute ());
  ...
  if (toDst.GetRreqCnt () == RreqRetries)
    {
      ...
      m_addressReqTimer.erase (dst);
      m_rankBuffer.DeleteRoute (dst);								//XJLM::TODO
...
{
  NS_LOG_FUNCTION (this);
  m_rankBuffer.MarkLinkAsUnidirectional (neighbor, blacklistTimeout);
}

void
RoutingProtocol::SendHello ()
{
  // cout << "\t\t\tTOTAL PACKETS RECEIVED :: " << m_ipv4->GetObject<Node>()->GetId() << " = [" << m_totalPacketsReceived << "]" << endl;
  // cout << "\t\t\tSUCCESSFUL DELIVERIES TO :: " << m_ipv4->GetObject<Node>()->GetId() << " = {" << m_successfullPacketsRecieved << "}" << endl;
...
RoutingProtocol::SendRerrWhenBreaksLinkToNextHop (Ipv4Address nextHop)
{
  ...
  RoutingTableEntry toNextHop;
  if (!m_rankBuffer.LookupRoute (nextHop, toNextHop))								//XJLM::TODO
    return;
  toNextHop.GetPrecursors (precursors);
  rerrHeader.AddUnDestination (nextHop, toNextHop.GetSeqNo ());
  m_rankBuffer.GetListOfDestinationWithNextHop (nextHop, unreachable);								//XJLM::TODO
  ...
  m_rankBuffer.InvalidateRoutesWithDst (nextHop, unreachable);								//XJLM::TODO
}
...
RoutingProtocol::SendRerrMessage (Ptr<Packet> packet, std::vector<Ipv4Address> precursors)
{
  ...
  if (precursors.size () == 1)
    {
      ...
      if (m_rankBuffer.LookupValidRoute (precursors.front (), toPrecursor))								//XJLM::TODO
        {
  ...
  for (std::vector<Ipv4Address>::const_iterator i = precursors.begin (); i != precursors.end (); ++i)
    {
      if (m_rankBuffer.LookupValidRoute (*i, toPrecursor) && 								//XJLM::TODO
