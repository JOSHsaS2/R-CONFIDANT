...
class RoutingProtocol : public Ipv4RoutingProtocol
{
    ...
    friend class ns3::confidant::Monitor;
    friend class ns3::confidant::ReputationSystem;
    friend class ns3::confidant::PathManager;
    friend class ns3::confidant::TrustManager;

private:
    bool ifEnableC;//initial false
    bool ifEnableL;//initial false, when set L as TRUE, C is TRUE too
    Ptr<ns3::confidant::PathManager> pathM;
    Ptr<ns3::confidant::TrustManager> trustM;
    Ptr<ns3::confidant::ReputationSystem> repuSys;
    Ptr<ns3::confidant::Monitor> c_monitor;///change invoker in aodv.h/cc and test script



    //CONFIDANT: Set callback for m_ipv4 for registing outbound packets
    //set when setC (true);
    void setCallbackForRegisterOutPacket()
    {
        (DynamicCast<Ipv4L3Protocol>(m_ipv4))->setCallbackForRegister(MakeCallback(&ns3::confidant::Monitor::registerPack, c_monitor));
    }

public:
    //set reputationsystem public_t timer
    void setC(bool c)
    {
        ifEnableC = c;
        if(ifEnableC)
        {
            repuSys->setPublicTimer();
            setCallbackForRegisterOutPacket();
        }
    ...
    {
        ifEnableL = l;
        c_monitor->setEableL(ifEnableL);
        repuSys->setEableL(ifEnableL);
    }

    void setT(bool t)
    {
        repuSys->setTradeoffSwitch(t);
    }

    //return pointer of monitor for set promis callback in wifinetdevice
    Ptr<ns3::confidant::Monitor> getMonitor()
    {
        return c_monitor;
    }

    void setIsNeighborFlag(bool isneig){
    	isNeighbor = isneig;
    }
...
public:
    ...
    {
        IsMalicious = f;    // SELFISH: Method declared for Blackhole Attack Simulation - Shalini Satre
    }
    bool GetMaliciousEnable () const
    {
        return IsMalicious;    // Method declared for Blackhole Attack Simulation - Shalini Satre
    }
...
private:
    ...
    bool isNeighbor; // variable for avoid dead loop when call senderrorcallback when node is deteced as selfish.
