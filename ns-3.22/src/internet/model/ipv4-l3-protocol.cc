...
// it under the terms of _the GNU General Public License version 2 as
...
void Ipv4L3Protocol::Send(Ptr<Packet> packet, Ipv4Address source,
	...
	if (route && route->GetGateway() != Ipv4Address()) {
		...
		//CONFIDANT: Callback to register outbound packets into packQueue.
		if (!callbackForRegister.IsNull()) {
			Ptr<Ipv4Interface> outInterface = GetInterface(interface);
			if (outInterface->GetDevice()->NeedsArp()) {
				Address mac_id;
				if (!(outInterface->GetArpCache()))
					return;
				else {
					ArpCache::Entry *entry =
							(outInterface->GetArpCache())->Lookup(
									route->GetGateway());
					if (entry != 0 && !(entry->IsExpired())
							&& entry->IsAlive()) {
						mac_id = entry->GetMacAddress();
						NS_LOG_UNCOND("[" << m_node->GetId() << "]" <<
								"Register a outbound packet in Ipv4-L3-Protocol: Send()");
						NS_LOG_UNCOND(mac_id <<" "<< route->GetGateway()<<" " << destination);
						callbackForRegister(packet->Copy(), mac_id, destination,
								route->GetGateway());
					}
				}
			}
	...
	if (newRoute) {
		...
		if (!callbackForRegister.IsNull()) {
			Ptr<Ipv4Interface> outInterface = GetInterface(interface);
			//Address mac_id = ((outInterface->GetArpCache())->Lookup(newRoute->GetGateway()))->GetMacAddress();
			if (outInterface->GetDevice()->NeedsArp()) {
				Address mac_id;
				if (!(outInterface->GetArpCache()))
					return;
					...
					if (entry != 0 && !(entry->IsExpired())
						...
						mac_id = entry->GetMacAddress();
						NS_LOG_UNCOND(
								"[" << m_node->GetId() << "]"<< "Register a outbound packet in Ipv4-L3-Protocol: Send()");
						NS_LOG_UNCOND(mac_id <<" "<< newRoute->GetGateway()<<" " << destination);
						callbackForRegister(packet->Copy(), mac_id, destination,
								newRoute->GetGateway());
					}
				}
