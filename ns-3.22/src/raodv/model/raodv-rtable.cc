...
RoutingTable::LookupRoute (Ipv4Address id, RoutingTableEntry & rt)
{
  ...
  if (m_ipv4AddressEntry.empty ())
    {
      NS_LOG_LOGIC ("RTABLE :: Route to " << id << " not found; m_ipv4AddressEntry is empty");
      return false;
    }
  ...
  if (i == m_ipv4AddressEntry.end ())
    {
      NS_LOG_LOGIC ("RTABLE :: Route to " << id << " not found");
      return false;
    }
  rt = i->second;
  NS_LOG_LOGIC ("RTABLE :: Route to " << id << " found");
  return true;
}
...
RoutingTable::LookupValidRoute (Ipv4Address id, RoutingTableEntry & rt)
{
  ...
  NS_LOG_LOGIC ("RTABLE :: Route to " << id << " flag is " << ((rt.GetFlag () == VALID) ? "valid" : "not valid"));
...
RoutingTable::DeleteRoute (Ipv4Address dst)
{
  ...
  if (m_ipv4AddressEntry.erase (dst) != 0)
    {
      NS_LOG_LOGIC ("RTABLE :: Route deletion to " << dst << " successful");
      return true;
    }
  NS_LOG_LOGIC ("RTABLE :: Route deletion to " << dst << " not successful");
  return false;
}
...
RoutingTable::Update (RoutingTableEntry & rt)
{
  ...
  if (i == m_ipv4AddressEntry.end ())
    {
      NS_LOG_LOGIC ("RTABLE :: Route update to " << rt.GetDestination () << " fails not found");
      return false;
    }
  i->second = rt;
  if (i->second.GetFlag () != IN_SEARCH)
    {
      NS_LOG_LOGIC ("RTABLE :: Route update to " << rt.GetDestination () << " set RreqCnt to 0");
