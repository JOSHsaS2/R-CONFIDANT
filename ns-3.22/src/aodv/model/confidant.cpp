...
void
PackForAck::suspendTimer(){
   timeout.Suspend();
}

void
PackForAck::resumeTimer(){
	timeout.Resume();
}

PackForAck::PackForAck(Ptr<const Packet> pack, Address mach, Ipv4Address iph):
    p(pack),
    timeout(ns3::Timer::CANCEL_ON_DESTROY),
    macOfNextHop(mach),
    ipOfNextHop(iph)
{
}
...
double
Repu_Rating::getThreshold()
{
    return threshold;
}

void
Repu_Rating::setThreshold(double thres)
{
    threshold = thres;
}
...
void
Repu_Rating::updateByOb(BEHAVIOR behavior)
{
    fadeRating();
...
void
Repu_Rating::updateReputation(double salpha,double sbeta, double w)
{
    alpha += w * salpha;
    beta += w * sbeta;
}
...
Rating(fading_f),
f_timer(Timer::CANCEL_ON_DESTROY)
{
...
void
F_Rating::setRepu(Repu_Rating* r)
{
    repu = r;
}

void
F_Rating::cancelTimer()
{
    f_timer.Cancel();
}

void
F_Rating::reScheduleTimer()
{
    f_timer.Schedule(Time(Seconds(FADING_TIMEOUT)));
...
void
F_Rating::updateByOb(BEHAVIOR behavior)
{
    fadeRating();
    int s = ((behavior == regular)? 0:1);
    alpha += s;
...
void
F_Rating::handleFadingTimeoutExpire(Ipv4Address f_ip)
{
    fadeRating();
    repu -> fadeRating();///?
    //NS_LOG_DEBUG("Fading rating item for first and reputation!");
    //reputation redemption mechanism here
    classifyNode(f_ip, this, repu);
    f_timer.Cancel();
    reScheduleTimer();
}

void
F_Rating::setClassifyCallback(Callback<void, Ipv4Address, F_Rating*, Repu_Rating*> cb)
{
    classifyNode = cb;
}
...
f_timer(Timer::CANCEL_ON_DESTROY)
{
    ...
    f_timer.Schedule(Time(Seconds(FADING_TIMEOUT)));
    NS_LOG_DEBUG("Initiate timer for trust rating item at time: " +
    		NumberToString(Simulator::Now().ToDouble(ns3::Time::MS)));
}

void
Tru_Rating::cancelTimer()
{
    f_timer.Cancel();
}

void
Tru_Rating::reScheduleTimer()
{
    f_timer.Schedule(Time(Seconds(FADING_TIMEOUT)));
}

void
Tru_Rating::handleFadingTimeoutExpire()
{
    fadeRating();
    //f_timer.Cancel();
    reScheduleTimer();
    NS_LOG_DEBUG("Reschedule new timer for trust rating item at time: " +
    		NumberToString(Simulator::Now().ToDouble(ns3::Time::MS)));
}

void
Tru_Rating::updateByOb(DEVIATIONTEST result)
{
    fadeRating();
