...
{
  RaodvHelper raodv;
  // you can configure RRAODV attributes here using raodv.Set(name, value)
  ...
  if (printRoutes)
    {
      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("raodv.routes", std::ios::out);
      raodv.PrintRoutingTableAllAt (Seconds (8), routingStream);
    }
}
