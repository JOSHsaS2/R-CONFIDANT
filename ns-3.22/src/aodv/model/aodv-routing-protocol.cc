...
RoutingProtocol::RoutingProtocol() :
		///for test script
		ifEnableC(false), ifEnableL(false), RreqRetries(2), RreqRateLimit(10), RerrRateLimit(
				10), ActiveRouteTimeout(Seconds(3)), NetDiameter(35), NodeTraversalTime(
...
///add Ipv4 obejcet m_ipv4 into all constructors of CONFIDANT
//pathM(m_routingTable, m_ipv4),
//trustM(m_ipv4),
//repuSys(),
//c_monitor(repuSys, m_nb, PACK_TIMEOUT, m_ipv4)
{
	...
	pathM = CreateObject<ns3::confidant::PathManager>();
	trustM = CreateObject<ns3::confidant::TrustManager>();
	repuSys = CreateObject<ns3::confidant::ReputationSystem>();
	repuSys->setComponentsFromConfidant(trustM, pathM);
	c_monitor = CreateObject<ns3::confidant::Monitor>(); ///dont forget to change the use of pointers of confidnat in aodv.cc
	c_monitor->setReputationSystem(repuSys);

	///set callbacks for components in CONFIDANT
	repuSys->setCallbackSendTo(MakeCallback(&RoutingProtocol::SendTo, this));
	repuSys->setCallbackFindSocketIp(
			MakeCallback(&RoutingProtocol::FindSocketWithInterfaceAddress,
					this));
	repuSys->setCallbackSendRerrWhenBreaksLinkToNextHop(
			MakeCallback(&RoutingProtocol::SendRerrWhenBreaksLinkToNextHop,
					this));
	...
	repuSys->setcallbackSendRequest(
			MakeCallback(&RoutingProtocol::SendRequest, this));

	///Aggregate CONFIDANT models to AODV::routingprotocol for config::connect
	///change arguments of pointers
	this->AggregateObject(c_monitor);
	this->AggregateObject(repuSys);
	this->AggregateObject(trustM);
	this->AggregateObject(pathM);
...
bool RoutingProtocol::Forwarding(Ptr<const Packet> p, const Ipv4Header & header,
	...
	NS_LOG_FUNCTION(this << p->GetUid() << header);
	Ipv4Address dst = header.GetDestination();
	Ipv4Address origin = header.GetSource();
	...
	if (m_routingTable.LookupRoute(dst, toDst)) {
		if (toDst.GetFlag() == VALID) {
			...
			ucb(route, p, header);
			//CONFIDANT: register a forwarded packet
			/*
			 use ipv4 to get mac address of next Hop ip address before invoking registerPack(),
			 to achieve this, add codes in function which could invoke the method, e.g. RouteInput() or
			 RouteOutput() etc..
			 */
			if (ifEnableC) {
				int32_t id = m_ipv4->GetInterfaceForDevice(
						toDst.GetOutputDevice());
				Ptr<Ipv4Interface> interface_id = (DynamicCast<Ipv4L3Protocol>(
						m_ipv4))->GetInterface(id);
				//Address mac_id = ((interface_id->GetArpCache())->Lookup(toDst.GetNextHop()))->GetMacAddress();
				if (interface_id->GetDevice()->NeedsArp()) {
					if (!(!(interface_id->GetArpCache()))) {
						Address mac_id;
						ns3::ArpCache::Entry *entry =
								(interface_id->GetArpCache())->Lookup(
										toDst.GetNextHop());
						if (entry != 0 && !(entry->IsExpired())
								&& entry->IsAlive()) {
							NS_LOG_APPEND_CONTEXT
							NS_LOG_UNCOND(
...
void RoutingProtocol::SetIpv4(Ptr<Ipv4> ipv4) {
	...
	///set aodv components to confidant
	pathM->setComponentsFromAodv(&m_routingTable, m_ipv4);
	trustM->setComponentsFromAodv(m_ipv4);
	repuSys->setComponentsFromAodv(&m_nb, m_ipv4, m_uniformRandomVariable,
			&m_socketAddresses);
	c_monitor->setComponentsFromAodv(&m_nb, m_ipv4);
...
void RoutingProtocol::RecvAodv(Ptr<Socket> socket) {
	...
	case AODVTYPE_PRATING: {
		//!< MONITOR::handlePublicRating(Ipv4Address from, Ptr<const Packet> p);
		//CONFIDANT
		c_monitor->handlePublicRating(sender, packet);
		break;
	}
	case AODVTYPE_ALARM: {
		//CONFIDANT: RecvAlarm
		repuSys->RecvAlarm(packet, m_nb, m_routingTable, AODV_PORT);
		break;
	}
	}
}
...
void RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver,
	...
	if (/*IsMalicious || */m_routingTable.LookupRoute(dst, toDst)) {
		...
		if (/*IsMalicious ||*/((rreqHeader.GetUnknownSeqno()
				...
				&& toDst.GetValidSeqNo())) {
			if (/*IsMalicious || */(!rreqHeader.GetDestinationOnly()
					&& toDst.GetFlag() == VALID)) {
				m_routingTable.LookupRoute(origin, toOrigin);
	...
	if (m_routingTable.LookupRoute(dst, toDst)) {
		...
		// (ii)the Destination Sequence Number in the RREP is greater than the node's copy of the destination sequence number and the known value is valid,
			...
			if ((rrepHeader.GetDstSeqno() == toDst.GetSeqNo())
				...
				} else if (ifEnableC && !ifEnableL) {
					repuSys->updateRouteTableFromRREPC(m_routingTable,
							rrepHeader, toDst, newEntry, sender);
				} else if (ifEnableL) {
					repuSys->updateRouteTableFromRREPL(m_routingTable,
							rrepHeader, toDst, newEntry, sender);
				}
			}
...
// CONFIDANT: add code to send alarm from queue
void RoutingProtocol::SendPacketFromQueue(Ipv4Address dst,
	...
	while (m_queue.Dequeue(dst, queueEntry)) {
		DeferredRouteOutputTag tag;
		ns3::confidant::DeferredAlarmTag alarm_tag;
		...
		if (ifEnableL && p->RemovePacketTag(alarm_tag)) {
			...
			if (ifroute) {
				NS_LOG_APPEND_CONTEXT
				NS_LOG_UNCOND("send alarm from RREQ queue!");
				repuSys->SendAlarmFromQueue(p, rt, AODV_PORT);
				continue;
			} else
				NS_ASSERT(ifroute);
		}
		...
		//CONFIDANT: register a forwarded packet (both data packets of others an self)
		if (ifEnableC) {
			int32_t id = m_ipv4->GetInterfaceForDevice(
					route->GetOutputDevice());
			Ptr<Ipv4Interface> interface_id = (DynamicCast<Ipv4L3Protocol>(
					m_ipv4))->GetInterface(id);
			//Address mac_id = ((interface_id->GetArpCache())->Lookup(
			//route->GetGateway()))->GetMacAddress();
			if (interface_id->GetDevice()->NeedsArp()) {
				if (!(!(interface_id->GetArpCache()))) {
					Address mac_id;
					ns3::ArpCache::Entry* entry =
	...
	if (ifEnableL) {
		...
		if (repuSys->isMalicious(nextHop)){
			...
			if (!repuSys->getTradeoffSwitch()) {
				...
				repuSys->rmReputationItem(nextHop);
				//CONFIDANT: the name of object of PathManager is pathM
				pathM->rmMnode(nextHop);
			}
		}
	}
}
